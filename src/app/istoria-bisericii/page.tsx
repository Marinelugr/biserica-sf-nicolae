...
const historyArticle = {
  intro: `Biserica Sfântul Ierarh Nicolae din Hîrtopul Mic, Raionul Criuleni, este unul dintre lăcașurile de cult cu o istorie îndelungată din ținuturile Moldovei. Întemeiată în secolul al XIX-lea, biserica a fost martoră a nenumărate generații de credincioși care și-au botezat pruncii, și-au cununat tinerii și și-au odihnit pe cei adormiți.`,
  sections: [
    {
...
// Galerie foto — imagini din /public/images/ (admin poate adăuga mai multe)
const GALLERY: { src: string; alt: string }[] = [
  { src: '/images/12.jpg', alt: 'Vedere aeriană a Bisericii Sfântul Ierarh Nicolae' },
  // Imagini suplimentare vor fi adăugate de admin
]

// Video-uri istorice — link-uri YouTube/Vimeo (admin poate adăuga)
const VIDEOS: { url: string; title: string; embed: string }[] = [
  // Exemplu: { url: 'https://youtube.com/watch?v=...', title: 'Sfințirea Altarului 2020', embed: 'https://www.youtube.com/embed/...' }
]

export default function IstoriaBisericiiPage() {
  return (
    <div>
      {/* Banda panoramică 21:9 — fără text deasupra */}
      <div className="w-full overflow-hidden" style={{ aspectRatio: '21/9', position: 'relative', maxHeight: '420px' }}>
        ...
        <div
          ...
          style={{ background: 'linear-gradient(to bottom, rgba(7,4,1,0.1) 0%, rgba(7,4,1,0.45) 100%)' }}
        />
      </div>
      ...
      <div className="py-14 px-4 text-center" style={{ backgroundColor: '#0D0905', borderBottom: '1px solid #1E1208' }}>
        ...
        <h1
          ...
          style={{ color: '#C9A84C', fontSize: 'clamp(28px, 4vw, 48px)', fontWeight: 400 }}
        >
          Istoria Bisericii
        </h1>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <p className="font-body text-lg leading-relaxed mb-12 text-center italic" style={{ color: '#4A3020' }}>
          {historyArticle.intro}
        </p>

        <div className="h-px mb-12" style={{ backgroundColor: '#E8E5E0' }} />

        <div className="space-y-12">
          {historyArticle.sections.map((section, i) => (
            <article key={i}>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          {GALLERY.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {/* Placeholder slot admin */}
              <div
                className="rounded-lg flex items-center justify-center"
                style={{ aspectRatio: '4/3', border: '2px dashed #D4C8A0', backgroundColor: '#FAFAF8' }}
              >
                <div className="text-center p-4">
                  <span style={{ color: '#D4C8A0', fontSize: '28px' }} aria-hidden="true">+</span>
                  <p className="font-body text-xs mt-1" style={{ color: '#C0B090' }}>Admin: adaugă foto</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
          ...
          {VIDEOS.length === 0 ? (
            <div
              ...
              style={{ backgroundColor: '#F7F3EC', border: '2px dashed #D4C8A0' }}
            >
              <span style={{ color: '#D4C8A0', fontSize: '40px' }} aria-hidden="true">▶</span>
              <p className="font-body text-sm mt-3" style={{ color: '#8A7050' }}>
                Videoclipurile vor fi adăugate de administrator din link-uri YouTube sau Vimeo.
              </p>
            </div>
          ) : (
